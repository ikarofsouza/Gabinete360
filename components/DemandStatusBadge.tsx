...
const DemandStatusBadge: React.FC<Props> = ({ status, className = "" }) => {
  const config: Record<DemandStatus, { label: string; classes: string }> = {
    ...
    ANALYSIS: { label: 'Análise Tática', classes: 'bg-indigo-100 text-indigo-700 border-indigo-300' },
    IN_PROGRESS: { label: 'Em Trâmite', classes: 'bg-blue-600 text-white border-blue-700 shadow-sm shadow-blue-100' },
    WAITING_THIRD_PARTY: { label: 'Pendente Terceiros', classes: 'bg-purple-100 text-purple-700 border-purple-300' },
    SUCCESS: { label: 'Sucesso', classes: 'bg-emerald-600 text-white border-emerald-700 shadow-sm shadow-emerald-100' },
