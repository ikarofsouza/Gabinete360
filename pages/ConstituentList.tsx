...
import { 
  ...
  Navigation, ShieldAlert, Database, CheckCircle, Save
} from 'lucide-react';
...
import { Constituent, Demand } from '../types';
...
const ConstituentList: React.FC = () => {
  ...
  const processImport = async () => {
    ...
    try {
      for (let i = 0; i < csvData.length; i++) {
        ...
        try {
          ...
          batch.push({
            ...
            address: { 
              zip_code: foundCep, street, number: row[columnMapping['number']] || '', complement: '', 
              neighborhood: (row[columnMapping['neighborhood']] || '').toUpperCase(), city, state 
            },
      ...
      await fetchData();
      showToast(`${batch.length} registros importados com sucesso!`);
    ...
    <div className="p-8 space-y-6 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={handleNewConstituent} className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-blue-200 hover:bg-blue-700 active:scale-95 transition-all">
              ...
              {table.getHeaderGroups().map(headerGroup => (
                <tr key={headerGroup.id} className="bg-slate-50/50 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                  {headerGroup.headers.map(header => (
                    <th key={header.id} className="px-8 py-6">{flexRender(header.column.columnDef.header, header.getContext())}</th>
                  ))}
                </tr>
              ))}
            </thead>
              ...
              {loading ? (
                ...
                <tr key={row.id} className="hover:bg-slate-50/80 transition-all group">
                  {row.getVisibleCells().map(cell => (
                    <td key={cell.id} className="px-8 py-6">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>
                  ))}
                </tr>
              )) : (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-10 border border-slate-100 space-y-6">
            <div className="flex items-center gap-4 text-amber-600 mb-2">
              <div className="w-12 h-12 rounded-2xl bg-amber-50 flex items-center justify-center shadow-inner"><ShieldAlert size={28} /></div>
              <div>
            ...
            <div className="space-y-4">
              ...
              <textarea rows={4} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium outline-none focus:ring-4 focus:ring-amber-500/10 transition-all resize-none" placeholder="Ex: Registro duplicado, solicitação do cidadão..." value={deletionReason} onChange={(e) => setDeletionReason(e.target.value)} />
            </div>
      ...
      {/* Componente Modal Sincronizado */}
      <ConstituentCreateModal 
        isOpen={isModalOpen} 
        onClose={() => { setIsModalOpen(false); setEditingConstituent(null); }} 
        onSuccess={fetchData} 
        ...
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-white shadow-2xl z-50 overflow-y-auto animate-in slide-in-from-right duration-300 border-l border-slate-200 flex flex-col">
              <div className="p-8 border-b border-slate-100 bg-white sticky top-0 z-20">
                 ...
                 <div className="flex items-center gap-3">
                    <a href={`https://wa.me/${selectedConstituent.mobile_phone}`} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-100"><MessageCircle size={18} /> WhatsApp</a>
                    <button onClick={() => handleEdit(selectedConstituent)} className="p-3.5 bg-slate-100 text-slate-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all"><Edit2 size={18} /></button>
                 </div>
              ...
              <div className="flex-1 p-10 overflow-y-auto custom-scrollbar">
                {activeTab === 'profile' && (
                  <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-3 gap-6">
                      ...
                      <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 space-y-2">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Carteira</p>
                        <p className="text-xs font-black text-blue-600 uppercase">{getResponsibleUser(selectedConstituent.responsible_user_id)?.name.split(' ')[0] || 'Gabinete'}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><StickyNote size={14} /> Notas Internas</p>
                        {tempNotes !== (selectedConstituent.notes || '') && (
                          <button onClick={handleSaveQuickNotes} disabled={isSavingNotes} className="px-3 py-1 bg-blue-600 text-white text-[9px] font-black uppercase rounded-lg shadow-lg hover:bg-blue-700 transition-all flex items-center gap-1.5">{isSavingNotes ? <Loader2 size={10} className="animate-spin" /> : <Save size={10} />} Salvar</button>
                        )}
                      </div>
                      <textarea rows={6} className="w-full bg-slate-50 p-6 rounded-[2rem] border border-slate-100 text-sm text-slate-600 outline-none focus:ring-4 focus:ring-blue-500/5 focus:bg-white transition-all resize-none italic" placeholder="Anotações estratégicas..." value={tempNotes} onChange={(e) => setTempNotes(e.target.value)} />
                    </div>
                  </div>
                )}
                {activeTab === 'location' && (
                    ...
                    <div className="bg-slate-900 rounded-[3rem] p-8 text-white relative overflow-hidden shadow-2xl">
                       ...
                       <div className="relative z-10 space-y-6">
                          ...
                          <div className="grid grid-cols-2 gap-8 pt-4">
                            ...
                            <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">CEP</p><p className="text-lg font-black uppercase">{maskCEP(selectedConstituent.address.zip_code)}</p></div>
                          </div>
                       </div>
                    </div>
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${selectedConstituent.address.street}, ${selectedConstituent.address.number}, ${selectedConstituent.address.neighborhood}`)}`} target="_blank" rel="noreferrer" className="flex items-center justify-between p-6 bg-white border border-slate-200 rounded-3xl hover:border-blue-500 transition-all shadow-sm">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center"><Navigation size={24} /></div>
                        <div><p className="text-sm font-black text-slate-900">Visualizar Logística</p><p className="text-[10px] font-bold text-slate-400 uppercase">Planejamento de Visitas</p></div>
                      </div>
                ...
                {activeTab === 'history' && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex items-center justify-between"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><ClipboardList size={14} /> Demandas</p><span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-[10px] font-black">{constituentDemands.length} REGISTROS</span></div>
                    <div className="space-y-4">
              ...
              <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                 <button onClick={() => handleDelete(selectedConstituent.id)} className="flex items-center gap-2 px-6 py-3 bg-white text-rose-600 rounded-xl text-[10px] font-black uppercase tracking-widest border border-rose-100 hover:bg-rose-50 transition-all"><Trash2 size={16} /> Excluir Registro</button>
                 <button onClick={() => setIsDrawerOpen(false)} className="px-8 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all">Fechar Ficha</button>
              </div>
          </div>
        </>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-in fade-in">
          <div className="bg-white w-full max-w-4xl rounded-[3rem] p-10 shadow-2xl space-y-8 overflow-y-auto max-h-[95vh] border border-white/20">
             <div className="flex items-center justify-between border-b border-slate-100 pb-8">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-blue-600 text-white rounded-[1.5rem] flex items-center justify-center shadow-2xl"><FileSpreadsheet size={32} /></div>
                  <div><h3 className="text-2xl font-black text-slate-900 uppercase">Smart Import v2</h3><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">IA Integrada para Tags e Geolocalização</p></div>
                </div>
             ...
             {importStep === 0 ? (
                <div className="space-y-10 animate-in slide-in-from-bottom-4">
                    <div onClick={() => fileInputRef.current?.click()} className="group cursor-pointer bg-slate-50 border-4 border-dashed border-slate-200 rounded-[3rem] p-16 flex flex-col items-center justify-center gap-6 hover:bg-blue-50/50 hover:border-blue-200 transition-all">
                      <div className="w-20 h-20 bg-white rounded-3xl shadow-xl flex items-center justify-center text-slate-300 group-hover:text-blue-500 transition-all"><Upload size={36} /></div>
                      <div className="text-center"><p className="text-lg font-black text-slate-900 mb-1">Selecione o arquivo .CSV</p><p className="text-sm text-slate-400 font-medium">Arraste aqui ou clique para buscar</p></div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                    {[
                      ...
                      <div key={field.key} className="flex items-center justify-between group">
                        <div className="flex items-center gap-3">
                           <div className="p-2.5 bg-slate-50 text-slate-400 rounded-xl group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors"><field.icon size={16} /></div>
                           <span className="text-xs font-black text-slate-700 uppercase tracking-tight">{field.label}</span>
                        </div>
                        <select className={`w-48 bg-slate-50 border px-4 py-2.5 rounded-xl text-[10px] font-black uppercase outline-none cursor-pointer ${columnMapping[field.key] ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200'}`} value={columnMapping[field.key] || ''} onChange={(e) => setColumnMapping(prev => ({ ...prev, [field.key]: e.target.value }))}><option value="">-- Ignorar --</option>{csvHeaders.map(h => <option key={h} value={h}>{h}</option>)}</select>
                      </div>
                    ))}
                  </div>
                  <div className="pt-8 border-t border-slate-100 flex gap-4">
                     <button onClick={() => setImportStep(0)} className="px-10 py-5 bg-slate-100 text-slate-500 font-black rounded-2xl text-[10px] uppercase">Voltar</button>
                     <button onClick={processImport} disabled={!columnMapping['name'] || !columnMapping['mobile_phone']} className="flex-1 py-5 bg-blue-600 text-white font-black rounded-2xl text-[10px] uppercase shadow-2xl flex items-center justify-center gap-3 disabled:opacity-50"><CheckCircle size={18} /> Iniciar Processamento Estratégico</button>
                  </div>
                </div>
             ) : (
                <div className="py-20 flex flex-col items-center text-center space-y-12 animate-in fade-in">
                    <div className="relative"><div className="w-48 h-48 border-[12px] border-slate-50 border-t-blue-600 rounded-full animate-spin"></div><div className="absolute inset-0 flex items-center justify-center"><p className="text-3xl font-black text-slate-900">{Math.round((importProgress.current / importProgress.total) * 100)}%</p></div></div>
                    <p className="text-lg font-black text-slate-900 uppercase animate-pulse">{importProgress.status}</p>
                </div>
             )}
          </div>
        </div>
